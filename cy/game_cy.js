// Hangmango Welsh game logic v=001

// ── Accent normalisation ────────────────────────────────────────────────
// Players press unaccented keys (A-Z). This strips Welsh circumflex accents
// so pressing "A" matches "â", "W" matches "ŵ", "Y" matches "ŷ" etc.
...
// Setup selections
let selectedCategory = 'anifeiliaid';
...
function guessLetter(letter) {
  ...
  // Normalised match: "A" matches "Â", "W" matches "Ŵ", etc.
  ...
  if (!wordHasLetter) {
    ...
    // Hyphens are auto-revealed — treat them as already guessed
    const allRevealed = [...state.word].every(c =>
      c === ' ' || c === '-' || state.guessed.has(normalise(c).toUpperCase())
    );
    if (allRevealed) {
      state.status = 'won';
...
function renderLives() {
  ...
  justLostIndex = -1;
  slotsEl.setAttribute('aria-label', `${state.wrongCount} o ${MAX_WRONG} camgymeriad`);

  label.className = '';
  if (remaining <= 0) {
    label.textContent = '';
  } else if (remaining === 1) {
    label.textContent = 'Cyfle olaf!';
    label.classList.add('warn-danger');
  } else if (remaining === 2) {
    label.textContent = `${remaining} ar ôl`;
...
function buildTile(char) {
  // Hyphens are auto-revealed — players don't need to guess them
  if (char === '-') {
    return `<span class="letter-tile revealed">-</span>`;
  }
...
function setCategoryHint(category, difficulty) {
  ...
  if (!category || category === 'custom') {
    hint.textContent = '✏️ Gair personol';
    return;
  }
  const meta = CATEGORY_META_CY[category];
  const diffLabels = { easy: 'Hawdd', medium: 'Canolig', hard: 'Anodd' };
...
function showResultModal(won) {
  ...
  playAgain.textContent  = won ? 'Nesaf 🥭' : 'Ceisio eto 🔄';

  const m = Math.floor(timerSeconds / 60);
  const s = timerSeconds % 60;
  const timeStr = `${m}:${s.toString().padStart(2, '0')}`;
  const wrongLabel = 'camgymeriad';
  ...
  if (won) {
    ...
    statsEl.innerHTML = `
      <div>💥 Y gair oedd <strong>${state.word}</strong></div>
      <div>${state.wrongCount} ${wrongLabel} &nbsp;·&nbsp; ⏱ ${timeStr}</div>
    `;
  }
  ...
  shareBtn.textContent = "Rhannu'r canlyniad 🥭";
  shareBtn.onclick = () => {
    const text = won
      ? `🥭 hangmango\nDyfalodd "${state.word}" gyda ${state.wrongCount} ${wrongLabel} mewn ${timeStr}!\nChwaraewch am ddim ar hangmango.net/cy/`
      : `💥 hangmango\nMethu dyfalu "${state.word}" — ${state.wrongCount} ${wrongLabel} mewn ${timeStr}!\nAllwch chi wneud yn well? hangmango.net/cy/`;
    if (navigator.share) {
      navigator.share({ text, url: 'https://hangmango.net/cy/' }).catch(() => {});
    } else {
      navigator.clipboard.writeText(text).then(() => {
        shareBtn.textContent = 'Wedi copïo! ✓';
        setTimeout(() => { shareBtn.textContent = "Rhannu'r canlyniad 🥭"; }, 2000);
      }).catch(() => {});
    }
  };
...
function playAgainSameSettings() {
  ...
  const word = state.category === 'custom'
    ? null
    : getWordCY(state.category, state.difficulty);
...
function initSetup() {
  ...
  document.getElementById('play-random-btn').addEventListener('click', () => {
    const word = getWordCY(selectedCategory, selectedDifficulty);
    showScreen('game-screen');
  ...
  document.getElementById('play-custom-btn').addEventListener('click', () => {
    ...
    if (raw.length < 2) {
      errEl.textContent = 'Rhowch o leiaf 2 lythyren!';
      return;
    }
    if (!/^[a-zA-ZâêîôûŵŷÂÊÎÔÛŴŶ\-\s]+$/.test(raw)) {
      errEl.textContent = 'Llythrennau yn unig, os gwelwch yn dda!';
      return;
    }
    errEl.textContent = '';
