...
function guessLetter(letter) {
  ...
  // Normalised match: "E" matches "é","è","ê", "C" matches "ç", etc.
...
function renderLives() {
  ...
  justLostIndex = -1;
  slotsEl.setAttribute('aria-label', `${state.wrongCount} de ${MAX_WRONG} erreurs`);

  label.className = '';
  if (remaining <= 0) {
    label.textContent = '';
  } else if (remaining === 1) {
    label.textContent = 'Dernière chance !';
...
function setCategoryHint(category, difficulty) {
  ...
  if (!category || category === 'custom') {
    hint.textContent = '✏️ Mot personnalisé';
    return;
  }
  const meta = CATEGORY_META_FR[category];
...
function showResultModal(won) {
  ...
  playAgain.textContent  = won ? 'Suivant 🥭' : 'Réessayer 🔄';

  const m = Math.floor(timerSeconds / 60);
  const s = timerSeconds % 60;
  const timeStr = `${m}:${s.toString().padStart(2, '0')}`;
  const wrongLabel = state.wrongCount === 1 ? 'erreur' : 'erreurs';
  ...
  if (won) {
    ...
    statsEl.innerHTML = `
      <div>💥 Le mot était <strong>${state.word}</strong></div>
      <div>${state.wrongCount} ${wrongLabel} &nbsp;·&nbsp; ⏱ ${timeStr}</div>
    `;
  }
  ...
  shareBtn.onclick = () => {
    const text = won
      ? `🥭 hangmango\nJ'ai deviné "${state.word}" avec ${state.wrongCount} ${wrongLabel} en ${timeStr} !\nJoue gratuitement sur hangmango.net/fr/`
      : `💥 hangmango\nJe n'ai pas deviné "${state.word}" — ${state.wrongCount} ${wrongLabel} en ${timeStr} !\nTu peux faire mieux ? hangmango.net/fr/`;
    if (navigator.share) {
      navigator.share({ text, url: 'https://hangmango.net/fr/' }).catch(() => {});
    } else {
      navigator.clipboard.writeText(text).then(() => {
        shareBtn.textContent = 'Copié ! ✓';
        setTimeout(() => { shareBtn.textContent = 'Partager le résultat 🥭'; }, 2000);
      }).catch(() => {});
    }
  };
...
function playAgainSameSettings() {
  ...
  const word = state.category === 'custom'
    ? null
    : getWordFR(state.category, state.difficulty);
...
function initSetup() {
  ...
  document.getElementById('play-random-btn').addEventListener('click', () => {
    const word = getWordFR(selectedCategory, selectedDifficulty);
    showScreen('game-screen');
  ...
  document.getElementById('play-custom-btn').addEventListener('click', () => {
    ...
    if (raw.length < 2) {
      errEl.textContent = 'Entre au moins 2 lettres !';
      return;
    }
    if (!/^[a-zA-ZàâéèêëîïôùûüçœæÀÂÉÈÊËÎÏÔÙÛÜÇŒÆ\s]+$/.test(raw)) {
      errEl.textContent = 'Lettres uniquement !';
      return;
    }
    errEl.textContent = '';
