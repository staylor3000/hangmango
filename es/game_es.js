...
function guessLetter(letter) {
  ...
  // Normalised match: "E" matches "É", "N" matches "Ñ", etc.
...
function renderLives() {
  ...
  justLostIndex = -1;
  slotsEl.setAttribute('aria-label', `${state.wrongCount} de ${MAX_WRONG} intentos fallados`);

  label.className = '';
  if (remaining <= 0) {
    label.textContent = '';
  } else if (remaining === 1) {
    label.textContent = '¡Última oportunidad!';
...
function setCategoryHint(category, difficulty) {
  ...
  if (!category || category === 'custom') {
    hint.textContent = '✏️ Palabra personalizada';
    return;
  }
  const meta = CATEGORY_META_ES[category];
...
function showResultModal(won) {
  ...
  playAgain.textContent  = won ? 'Siguiente 🥭' : 'Intentar de nuevo 🔄';

  const m = Math.floor(timerSeconds / 60);
  const s = timerSeconds % 60;
  const timeStr = `${m}:${s.toString().padStart(2, '0')}`;
  const wrongLabel = state.wrongCount === 1 ? 'error' : 'errores';
  ...
  if (won) {
    ...
    statsEl.innerHTML = `
      <div>💥 La palabra era <strong>${state.word}</strong></div>
      <div>${state.wrongCount} ${wrongLabel} &nbsp;·&nbsp; ⏱ ${timeStr}</div>
    `;
  }
  ...
  shareBtn.onclick = () => {
    const text = won
      ? `🥭 hangmango\nAdiviné "${state.word}" con ${state.wrongCount} ${wrongLabel} en ${timeStr}!\nJuega gratis en hangmango.net/es/`
      : `💥 hangmango\nNo pude adivinar "${state.word}" — ${state.wrongCount} ${wrongLabel} en ${timeStr}!\n¿Puedes hacerlo mejor? hangmango.net/es/`;
    if (navigator.share) {
      navigator.share({ text, url: 'https://hangmango.net/es/' }).catch(() => {});
    } else {
...
function playAgainSameSettings() {
  ...
  const word = state.category === 'custom'
    ? null
    : getWordES(state.category, state.difficulty);
...
function initSetup() {
  ...
  document.getElementById('play-random-btn').addEventListener('click', () => {
    const word = getWordES(selectedCategory, selectedDifficulty);
    showScreen('game-screen');
  ...
  document.getElementById('play-custom-btn').addEventListener('click', () => {
    ...
    if (raw.length < 2) {
      errEl.textContent = '¡Ingresa al menos 2 letras!';
      return;
    }
    if (!/^[a-zA-ZáéíóúüñÁÉÍÓÚÜÑ\s]+$/.test(raw)) {
      errEl.textContent = '¡Solo letras, por favor!';
      return;
    }
    errEl.textContent = '';
